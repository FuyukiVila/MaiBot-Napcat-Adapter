...
class RecvHandler:
    ...
    def check_allow_to_chat(self, user_id: int, group_id: Optional[int]) -> bool:
        ...
        if group_id:
            ...
            if global_config.list_type == "whitelist" and (user_id not in global_config.user_list and "any" not in global_config.user_list):
                logger.warning("用户不在白名单中，消息被丢弃")
                return False
            elif global_config.list_type == "blacklist" and (user_id in global_config.user_list or "any" in global_config.user_list):
